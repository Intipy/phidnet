...
###############################################
phidnet.CNN.set.filter(filter_number=1, input_channel=1, filter_size=5, stride=1, pad=0)   # 30 filter -> 30 channel
phidnet.CNN.set.activation(phidnet.activation.Relu())
phidnet.CNN.set.pooling(height=2, width=2, stride=2)
phidnet.CNN.set.layer(144)
...
############################################### Fit model
phidnet.CNN.model.fit(epoch=5, optimizer=SGD, batch=5000, val_loss=False, print_rate=1)   # Showing validation loss make fitting slow
